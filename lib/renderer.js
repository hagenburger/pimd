...
module.exports = class Renderer extends MarkdownItRenderer {
  constructor(document) {
    ...
    this.rules['processing_instruction_block'] = function(tokens, idx, options, env, renderer) {
      const token = tokens[idx]
      const example = new ProcessingInstruction(renderer, options, token.name, token.content)
      example.append()
    }

    this.rules['processing_instruction_inline'] = function(tokens, idx, options, env, renderer) {
      const token = tokens[idx]
      const example = new ProcessingInstruction(renderer, options, token.name, token.content)
      example.append()
    }
